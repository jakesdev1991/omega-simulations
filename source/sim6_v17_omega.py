...
- Parameter scan driver: scan over κ, fit H(z), record (γ, κ, χ², Δf/f).
Features:
...
- Parameter scan: κ-grid, calibrated γ, H(z) χ², UV ringdown shift Δf/f.
"""
import os
import json
import time
...
# ------------------------
SHOW_PLOTS = True # can be disabled via CLI (--no-gui)
...
phi_c_def = 0.1
# critical COD scale in ℓ_P(Φ); tunable
...
# ------------------------
# Shared Sim 5 constants
# ------------------------
SIM5_N_REGIONS = 200
SIM5_DT = 0.1
SIM5_PHI_VACUUM = 1.0
SIM5_PHI_CRITICAL = 0.1
SIM5_L_PLANCK_BASE = 1.0
...
def run_sim5_dynamics():
    ...
    # 1. Universe config
    N_REGIONS = SIM5_N_REGIONS
    DT = SIM5_DT
    TOTAL_FRAMES = 300
    # Local toy Omega constants
    PHI_VACUUM = SIM5_PHI_VACUUM
    PHI_CRITICAL = SIM5_PHI_CRITICAL
    L_PLANCK_BASE = SIM5_L_PLANCK_BASE
    KAPPA_dyn = 5.0
    G_dyn = 0.05
    INFO_PER_MASS = 1.0
    ...
    def update(frame):
        ...
        for i in range(len(positions)):
            ...
            if start < end:
                ...
                velocities = [velocities[i] for i in active_indices]
        # Info bookkeeping
        phi_field = phi
    ...
    # Optional: compare local A_BH_local with global A_BH(z) shape if available
    try:
        cosmo = np.load("sim6_v17_omega_cosmo.npz")
        A_global = cosmo["A_BH"]
        A_global_norm = A_global / np.max(A_global)
        A_local_norm = np.array(A_BH_local_history) / max(A_BH_local_history[0], 1e-12)
        x_local = t_arr / (t_arr[-1] if t_arr[-1] > 0 else 1.0)
        x_global = np.linspace(0, 1, len(A_global_norm))
        plt.figure(figsize=(6,4))
        plt.plot(x_local, A_local_norm, label="Local A_BH (Sim 5)", lw=2)
        plt.plot(x_global, A_global_norm, label="Global A_BH (cosmo, norm.)", lw=2, ls='--')
        plt.xlabel("Normalized time / scale")
        plt.ylabel("Normalized A_BH")
        plt.title("Sim 5 vs Cosmology: Local vs Global BH Area Growth (Shape)")
        plt.legend(); plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig("sim5_vs_cosmo_A_BH_shape.png", dpi=200)
    except Exception as e:
        print(f"Sim 5: Could not compare to cosmology A_BH (missing file?): {e}")
    # Metadata
    meta5 = {
    "version": "sim6_v17",
    "timestamp": time.time(),
    ...
    "L_PLANCK_BASE": L_PLANCK_BASE,
    "KAPPA_dyn": KAPPA_dyn,
    "G_dyn": G_dyn,
    "INFO_PER_MASS": INFO_PER_MASS,
    "deltaA_per_mass": deltaA_per_mass,
    }
    }
    with open("sim5_v1_metadata.json", "w") as f:
...
def run_sim5_v34_dynamics():
    ...
    fig.suptitle('Omega Protocol v3.4: Emergent Gravity & BH Shredding', color='white',
    fontsize=16)
    vel_history = []
    force_history = []
    time_history = []
...
def run_sim3_disformal(RUN_DYNAMIC=True):
    ...
    Static:
    - ℓ_P(φ) = exp((1 - φ)/2) [dimensionless toy, UV-focused]
    - C(φ) = exp(-2φ)
    ...
    """
    def lP_phi(phi):
        # Dimensionless dynamic Planck length (toy UV model)
    ...
    # Static ringdown-shift estimate
    phi0 = 0.0
    grad_phi0 = 0.1
    lP0 = lP_phi(phi0)
    ...
    if RUN_DYNAMIC:
        ...
        def phi_eom(t, y):
            phival, phidot = y
            # Clip φ for stability in this toy model
            phival_clipped = np.clip(phival, -5.0, 5.0)
            Hphi = H0 * np.exp(phival_clipped)
            phiddot = -3.0 * Hphi * phidot - m**2 * phival_clipped
        ...
        phi_num = sol.y[0]
        phidot_num = sol.y[1]
        H_num = H0 * np.exp(np.clip(phi_num, -5.0, 5.0))
        ...
        with open("sim3_metadata.json", "w") as f:
            json.dump(meta3, f, indent=2)

# ------------------------
# Cosmology + Ringdown runner
# ------------------------
def run_cosmology_and_ringdown(use_sim3_correction=True):
    """
    Run the Depletion cosmology, compute observables, and synthesize ringdown
    waveforms. Optionally apply a UV correction to ringdown frequencies based
    on Sim 3 output if available.
    """
    ...
    np.savez('sim6_v17_omega_cosmo.npz', **out)
    print("Saved cosmology output to sim6_v17_omega_cosmo.npz")
    # Simple diagnostic of d_info vs chi
    for z0 in [0.0, 1.0, 2.0]:
        idx = np.argmin(np.abs(out['z'] - z0))
        ratio = out['d_info_Mpc'][idx] / max(out['chi_Mpc'][idx], 1e-12)
        print(f"z={z0:.1f}: d_info/chi ≈ {ratio:.3f}")
    meta_cosmo = {
    "version": "sim6_v17",
    "timestamp": time.time(),
    "cosmo_params": {
    ...
    with open("sim6_v17_metadata.json", "w") as f:
        json.dump(meta_cosmo, f, indent=2)
    # 2) A_BH(t) for Ringdown
    t_gyr = out['t']
    A_rel = out['A_BH'] / (params['A0'] if params['A0']!=0 else 1.0)
    # 2.5) Optional UV correction from Sim 3
    sim3_correction = None
    if use_sim3_correction:
        try:
            sim3 = np.load("sim3_dynamic_run.npz")
            rd_shift = sim3["ringdown_shift_percent"]
            delta_f_over_f = np.max(rd_shift) / 100.0
            delta_f_over_f = float(np.clip(delta_f_over_f, -0.5, 0.5))
            sim3_correction = delta_f_over_f
            print(f"Applying Sim 3 UV correction to ringdown: Δf/f ≈ {delta_f_over_f:.3f}")
        except Exception as e:
            print(f"Sim 3 UV correction not applied (missing/invalid file): {e}")
    ...
    for ch in choices:
        ...
        if sim3_correction is not None:
            f_ring = f_ring * (1.0 + sim3_correction)
    ...
    np.savez("sim6_v17_ringdown_summary.npz", results=results_summary)
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("sim6_v17_omega_distance_metric.png", dpi=150)
    plt.close()
    print("Saved sim6_v17_omega_distance_metric.png")
    # 5) Cosmology summary
    ...
    plt.tight_layout()
    plt.savefig("sim6_v17_cosmo_summary.png", dpi=150)
    plt.close()
    print("Saved sim6_v17_cosmo_summary.png")

# ------------------------
# Parameter scan utilities
# ------------------------
def compute_chi2_Hz(model_out, z_data, H_data, H_err):
    """
    Compute chi-squared for H(z) against simple dataset.
    """
    H_model = np.interp(z_data, model_out['z'], model_out['H_km_s_Mpc'])
    return float(np.sum(((H_model - H_data) / H_err)**2))

def run_parameter_scan():
    """
    Minimal parameter scan:
    - Scan over κ values.
    - For each κ:
    * Calibrate γ to match H0.
    * Run cosmology (no plots).
    * Compute χ² vs synthetic H(z) (ΛCDM-like).
    * Record (κ, γ, χ², Δf/f (from Sim 3 if available), H0_model).
    - Save results to NPZ + CSV.
    """
    # Synthetic H(z) "data": ΛCDM-like with Ω_m = 0.315, Ω_Λ = 1 - Ω_m
    z_data = np.array([0.0, 0.2, 0.5, 0.8, 1.0, 1.5, 2.0])
    Omega_m = OMEGA_M_FID
    Omega_L = 1.0 - Omega_m
    H_data = H0_fid * np.sqrt(Omega_m*(1+z_data)**3 + Omega_L)
    H_err = np.full_like(z_data, 5.0) # 5 km/s/Mpc uncertainties
    # UV ringdown correction Δf/f from Sim 3 (if available)
    try:
        sim3 = np.load("sim3_dynamic_run.npz")
        rd_shift = sim3["ringdown_shift_percent"]
        delta_f_over_f = float(np.clip(np.max(rd_shift)/100.0, -0.5, 0.5))
        print(f"Parameter scan: using Δf/f from Sim 3: {delta_f_over_f:.3f}")
    except Exception as e:
        print(f"Parameter scan: Sim 3 UV correction not available, Δf/f set to 0.0 ({e})")
        delta_f_over_f = 0.0
    # κ grid (example; adjust as desired)
    kappa_grid = np.linspace(0.5, 2.0, 4) # [0.5, 1.0, 1.5, 2.0]
    results = []
    for kappa_val in kappa_grid:
        print(f"Scanning κ = {kappa_val:.3f} ...")
        params_template = {
        'alpha': alpha_def,
        ...
        gamma_calib = calibrate_gamma(params_template)
        params_template['gamma'] = gamma_calib
        sol = integrate_cosmo(params_template)
        out = compute_observables_from_solution(sol, params_template, zmax=2.0, nz=800)
        chi2 = compute_chi2_Hz(out, z_data, H_data, H_err)
        H0_model = out['H_km_s_Mpc'][0]
        print(f" -> γ = {gamma_calib:.3e}, H0_model = {H0_model:.2f}, χ² = {chi2:.2f}")
        results.append((kappa_val, gamma_calib, chi2, delta_f_over_f, H0_model))
    results_arr = np.array(
    results,
    dtype=[('kappa','f8'),('gamma','f8'),('chi2','f8'),('delta_f_over_f','f8'),('H0_model','f8')]
    )
    np.savez("sim6_v17_param_scan.npz", results=results_arr,
    z_data=z_data, H_data=H_data, H_err=H_err)
    with open("sim6_v17_param_scan.csv", "w") as f:
        f.write("kappa,gamma,chi2,delta_f_over_f,H0_model\n")
        for row in results_arr:
            f.write(f"{row['kappa']},{row['gamma']},{row['chi2']},"
            f"{row['delta_f_over_f']},{row['H0_model']}\n")
    print("Parameter scan complete. Saved sim6_v17_param_scan.npz and sim6_v17_param_scan.csv")

# ------------------------
# Full pipeline: Sim 3 → Cosmology+Ringdown → Sim 5s
# ------------------------
def full_pipeline():
    ...
    print("Running Depletion cosmology + ringdown (with Sim 3 UV correction)...")
    run_cosmology_and_ringdown(use_sim3_correction=True)
    print("Cosmology + ringdown complete.\n")
    print("Running Sim 5 v1 (local Ω-dynamics + metabolic inequality)...")
    run_sim5_dynamics()
    print("Sim 5 v1 complete.\n")
    print("Running Sim 5 v3.4-style (emergent gravity + GIF)...")
    run_sim5_v34_dynamics()
    print("Sim 5 v3.4-style complete.\n")

# ------------------------
# Backward-compatible main()
# ------------------------
def main():
    full_pipeline()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Omega Protocol Simulation Suite (sim6_v17)")
    parser.add_argument("--cosmo", action="store_true",
    help="Run Depletion cosmology + ringdown only.")
    parser.add_argument("--sim5v1", action="store_true",
    help="Run Sim 5 v1 (local Ω-dynamics).")
    parser.add_argument("--sim5v34", action="store_true",
    help="Run Sim 5 v3.4-style visualization.")
    parser.add_argument("--sim3", action="store_true",
    help="Run Sim 3 (disformal causality) only.")
    parser.add_argument("--scan", action="store_true",
    help="Run parameter scan over κ with H(z) χ².")
    parser.add_argument("--no-gui", action="store_true",
    help="Disable interactive plots (headless mode).")
    ...
    # import matplotlib
    # matplotlib.use("Agg")
    # If no flags provided, run full pipeline
    if not any([args.cosmo, args.sim5v1, args.sim5v34, args.sim3, args.scan]):
        full_pipeline()
    else:
        if args.cosmo:
            run_cosmology_and_ringdown(use_sim3_correction=True)
